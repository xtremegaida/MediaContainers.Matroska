...
namespace EBML.Matroska
{
   public class MatroskaTrackEntry
   {
      ...
      public int DefaultDecodedFieldDuration { get; set; }
      ...
      public void ReadFrom(EBMLMasterElement element)
      {
         ...
         foreach (var child in element.Children)
         {
            ...
            else if (child.Definition == MatroskaSpecification.DefaultDuration) { DefaultDuration = (int)child.IntValue; }
            else if (child.Definition == MatroskaSpecification.DefaultDecodedFieldDuration) { DefaultDecodedFieldDuration = (int)child.IntValue; }
      ...
      public async ValueTask Write(EBMLWriter writer, CancellationToken cancellationToken = default)
      {
         ...
         if (DefaultDuration > 0) { await writer.WriteUnsignedInteger(MatroskaSpecification.DefaultDuration, (ulong)DefaultDuration, cancellationToken); }
         if (DefaultDecodedFieldDuration > 0) { await writer.WriteUnsignedInteger(MatroskaSpecification.DefaultDecodedFieldDuration, (ulong)DefaultDecodedFieldDuration, cancellationToken); }
         if (MaxBlockAdditionID > 0) { await writer.WriteUnsignedInteger(MatroskaSpecification.MaxBlockAdditionID, (ulong)MaxBlockAdditionID, cancellationToken); }
      ...
      public EBMLMasterElement ToElement()
      {
         ...
         if (DefaultDuration > 0) { tracks.AddChild(new EBMLUnsignedIntegerElement(MatroskaSpecification.DefaultDuration, (ulong)DefaultDuration)); }
         if (DefaultDecodedFieldDuration > 0) { tracks.AddChild(new EBMLUnsignedIntegerElement(MatroskaSpecification.DefaultDecodedFieldDuration, (ulong)DefaultDecodedFieldDuration)); }
